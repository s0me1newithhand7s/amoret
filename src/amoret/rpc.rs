...
use discord_rich_presence::{
    ...
    activity::{Activity, Assets, Button, Party, Secrets, Timestamps, ActivityType as RpcActivityType}
};
...
pub async fn run() -> Result<(), Box<dyn std::error::Error>> {
    ...
    while rx.changed().await.is_ok() {
        let Some(cfg) = rx.borrow().clone() else { continue; };
...
fn build_activity(cfg: &DiscordConfig) -> Activity<'_> {
    ...
    if let Some(assets) = &cfg.assets {
        ...
        if let Some(img) = &assets.large_image { a = a.large_image(img.clone()); }
        if let Some(txt) = &assets.large_text { a = a.large_text(txt.clone()); }
        
        if let Some(img) = &assets.small_image { a = a.small_image(img.clone()); }
        if let Some(txt) = &assets.small_text { a = a.small_text(txt.clone()); }
        activity = activity.assets(a);
    }

    if let Some(ts) = &cfg.timestamps {
        let mut t = Timestamps::default();
        if let Some(start) = ts.start { t = t.start(start as i64); }
        if let Some(end) = ts.end { t = t.end(end as i64); }
        activity = activity.timestamps(t);
    }

    if let Some(party) = &cfg.party {
        let mut p = Party::default();
        if let Some(id) = &party.id { p = p.id(id.clone()); }
    ...
    if let Some(secrets) = &cfg.secrets {
        ...
        if let Some(j) = &secrets.join { s = s.join(j.clone()); }
        if let Some(sp) = &secrets.spectate { s = s.spectate(sp.clone()); }
        if let Some(m) = &secrets.r#match { s = s.r#match(m.clone()); }
